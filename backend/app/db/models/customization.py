...
class Customization(Base, TimestampMixin):
    ...
    jd_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("jds.id"), nullable=False
    )
    ...
    customized_resume: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    ...
    matched_resumes: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
